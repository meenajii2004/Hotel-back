...
const extractSearchDetails = async (userPrompt) => {
  ...
  try {
    ...
    const prompt = `Extract the city, price, rating, and amenities from the following text: ${userPrompt}. Output should be a JSON (either you pass or fail) in the format - {"priceOperator": "greater", "price": 100,ratingOperator": "greater", "rating": 5, "amenities": ["WiFi", "Pool", "Restaurant"], "city": "indore"}, if you did not find any key then just show an empty string but should be a JSON and no other text and in case of amenities an empty array, items in amenities can only contains these items : 'WiFi','Gym','Spa','Parking' 'Restaurant','Bar'. In priceOperator and ratingOperator only 1 out of these 4 values can be returned : 'greater', 'lower', 'equal', ''.`;
